...
if not os.path.exists(db_file):
    try:
        ...
        # (1, 'Рабочий'), (2, 'Администратор')
        position = [
            (1, 'Рабочий'),
            (2, 'Администратор')
        ]
        cursor.executemany(f"INSERT INTO positions(id_position, name_position) VALUES (?, ?);", position)
        conn.commit()

        # (1, 'Срочные задания')
        type_event = (1, 'Срочные задания')
        cursor.execute(f"INSERT INTO type_event(id_type, name_type) VALUES (?, ?);", type_event)
        conn.commit()

        id_name = (FIRST_USER_ID, 2, FIRST_USER_NAME)
        cursor.execute(f"INSERT INTO users(id_user, id_position_user, name) "
                       f"VALUES (?, ?, ?);", id_name)
        conn.commit()
        cursor.close()
